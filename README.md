...
[![PyPI version](https://img.shields.io/pypi/v/smboost)](https://pypi.org/project/smboost/)
...
agent = HarnessAgent(
    model="qwen3.5:2b",                             # any Ollama / HuggingFace model
    invariants=InvariantSuite.coding_agent(),
    scorer="adaptive",
    fallback_chain=["qwen3.5:2b", "qwen3.5:8b"],      # only escalates if needed
)

result = agent.run("Refactor this function to be async and add error handling")
print(result.output)
```
...
*early internal evals on coding/tool-calling tasks; public benchmark suite releasing Day 6.
...
result = agent.run("Write a REST API endpoint with input validation and error handling")
print(result.output)
# result.trace  → full step-by-step trace with confidence scores
# result.stats  → retry count, fallback triggers, total latency
```
